# utils\nextBuilder\frontend\components\music\music_content_component.py
import sys
import os
...
def create_music_content_component():
    component_content = f"""'use client'
import React, {{ useEffect, useState }} from 'react';
import {{ fetchMusic }} from '@/lib/utils/music';
    ...
    return (
        <main className="pt-20 flex min-h-screen flex-col items-center justify-start p-10 bg-gray-100">
            <div className="w-full mb-6">
                <h1 className="text-2xl font-bold text-gray-800">Music Content</h1>
            </div>
...
<div className="text-black">album: {{record.album}}</div>
<div className="text-black">artist: {{record.artist}}</div>
...
"""
    file_path = os.path.join(app_name, COMPONENT_DIR, 'MusicContent.jsx')
    write_to_file(file_path, component_content)

create_music_content_component()
