# Bot\build\code\cli\next\utils\nextBuilder\frontend\creator\components\creator_content_component.py
import sys
import os
...
def create_creator_content_components():
    # Load and filter entities for creator access from the manifest
    ...
    # Iterate over migration files to generate content components for creator entities
        ...
        if migration_file.endswith(".js"):
            ...
            if table_name in creator_entities:
                ...
                page_content = create_react_page_template(
                    table_name, 'Creator', grid_items, fields)

                # Determine the directory path for the creator component
                component_path = os.path.join(app_name,
                    COMPONENT_DIR, 'creator', table_name, f"{table_name.capitalize()}CreatorContent.jsx")
                
                write_to_file(
                    component_path, page_content)

if __name__ == "__main__":
    create_creator_content_components()
